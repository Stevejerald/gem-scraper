...
async def scrape_all():
    async with async_playwright() as p:
        ...
        print("-------------------------------------")
        print("🚀 Starting scraping (LIMIT = 6 pages)...\n")

        all_data = []

        # Scrape only first 6 pages
        MAX_PAGES = 6
        ...
        while page_no < MAX_PAGES:
            ...
            if not next_btn:
                print("\n⚠️ No next button. Stopping early.")
                break
...
if __name__ == "__main__":
    ...
    df.to_csv("gem_first_6_pages.csv", index=False)

    print("\n-------------------------------------")
    print(f"SCRAPED RECORDS:  {len(data)}")
    print(f"PAGES SCRAPED:    6")
    print("-------------------------------------")
    print(f"⏱ TOTAL TIME: {round(time.time() - start, 2)} seconds")
    print("📁 Saved: gem_first_6_pages.csv")
